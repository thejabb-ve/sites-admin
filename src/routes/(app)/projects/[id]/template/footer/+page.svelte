<script lang="ts">
  ...
  let currentProps = $state<Record<string, unknown>>(
    untrack(() => JSON.parse(JSON.stringify((data.project.footer_props as Record<string, unknown>) ?? {})) as Record<string, unknown>),
  );
...
<svelte:head><title>Pie de página — {data.project.name}</title></svelte:head>
...
<div class="space-y-6">
  <div class="flex items-center gap-2 text-sm text-gray-500 flex-wrap">
    ...
    <span>/</span>
    <span class="text-gray-900 font-medium">Pie de página</span>
  </div>

  <h1 class="text-xl font-semibold text-gray-900">Pie de página del sitio</h1>
  ...
  <form
    ...
    use:enhance={() => {
      ...
      return async ({ result, update }: { result: ActionResult; update: (opts?: { reset?: boolean }) => Promise<void> }) => {
        ...
        saving = false;
        if (result.type === 'success') toast('success', 'Pie de página guardado.');
  ...
  >
    ...
    {#each fields as field (field.key)}
      ...
      {@const inputId = `footer-${field.key}`}
      <div>
